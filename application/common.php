...
// +----------------------------------------------------------------------
const WX_PREFIX = 'https://api.weixin.qq.com/cgi-bin';
 ...
 * @return string
 */
function set_api(string $content):string
{
    return WX_PREFIX.$content;
}
 ...
 * @throws Exception
 */
function parse_json(string $response)
{
    $data = json_decode($response,true);

    if (isset($data['errcode']) && $data['errcode'] != 0){
        return get_wx_err_message($data);
    }else{
        if ($data){
            return $data;
        }else{
 ...
 * @return string
 */
function get_wx_err_message(array $data):string
{
    $err_message = [
        -1 => "系统繁忙，此时请开发者稍候再试",
        0 => "请求成功",
    ...
    return $err_message[$data['errcode']]??'错误码不存在,错误码:'.$data['errcode'].
        ',错误信息:'.$data['errmsg'];
}
